...
var src = fs.readFileSync(path.join(__dirname, '..', 'js/extensions/cave-facilities.js'), 'utf8');
...
// 无同伴的设施事件不应触发；空府琐事可以触发
assert(C.EMPTY_CAVE_EVENTS && C.EMPTY_CAVE_EVENTS.length === 4, '4 条空府琐事');
C.getState().caves['cave_4'] = { level: 'spirit_manor', facilities: [], companions: [], history: [] };
C.install('cave_4', 'fac_spirit_gathering', 0);
listeners = {};
var emptyHits = 0;
var companionHits = 0;
for (var dj = 0; dj < 400; dj++) {
    var te = C.tickDay('cave_4');
    for (var ek = 0; ek < te.events.length; ek++) {
        if (te.events[ek].empty) emptyHits++;
        else companionHits++;
    }
}
assert(companionHits === 0, '无同伴设施事件 0 (got ' + companionHits + ')');
assert(emptyHits >= 1, '空府琐事至少 1 (got ' + emptyHits + ')');
...
// ---- 9. 事件总线 ----
section('9) 事件总线');
assert(companionHits === 0, 'cave_4 无同伴设施事件仍为 0');
assert(emptyHits >= 1, 'cave_4 空府琐事已触发');
// 重新监听 cave_3
listeners = {};
