...
var mockWindow = {
    ...
    inventory: { currency: { spiritStones: 10000 } }
};
var fs = require('fs');
var path = require('path');
var src = fs.readFileSync(path.join(__dirname, '..', 'js/extensions/beast-tide.js'), 'utf8');
...
// 园 daysActive
M.build('sect_tick_a', { skipPay: true });
T.tickDay();
T.tickDay();
var gardens = G.listGardens('sect_tick_a');
log('  sect_tick_a 园数: ' + gardens.length);
assert(gardens.length === 1, '1 园');
assert(gardens[0].daysActive >= 2, 'daysActive ≥ 2');
var dupTick = M.build('sect_tick_a', { skipPay: true });
assert(!dupTick.ok && dupTick.reason === 'sect-garden-full', '同宗第二园拒');
var extraTick = M.build('sect_tick_a', { skipPay: true, allowMultiple: true });
assert(extraTick.ok, 'allowMultiple 可再建');
G.remove(extraTick.gardenId, { skipRefund: true });
G.remove(gardens[0].gardenId, { skipRefund: true });

// ---- 5. 灵兽园 ----
section('5) 灵兽园');
var stonesBefore = mockWindow.inventory.currency.spiritStones;
var b1 = G.build('sect_1');
assert(b1.ok, '建园 OK');
assert(b1.cost === 100, 'cost 100');
assert(mockWindow.inventory.currency.spiritStones === stonesBefore - 100, '扣 100 灵石');
assert((listeners['beast:gardenBuilt'] || []).length >= 1, 'built 事件');

var b2 = G.build('sect_2');
assert(b2.ok, '另一宗建园 OK');
var b2dup = G.build('sect_1');
assert(!b2dup.ok && b2dup.reason === 'sect-garden-full', '同宗再建拒');
...
assert(buff.trainingPct >= 0.2, '1 园 0.2 (got ' + buff.trainingPct + ')');
...
// remove
var stonesMid = mockWindow.inventory.currency.spiritStones;
var rm1 = G.remove(b1.gardenId);
assert(rm1 && rm1.ok, 'remove OK');
assert(rm1.refund === 50, '拆园退 50');
assert(mockWindow.inventory.currency.spiritStones === stonesMid + 50, '灵石退回');
var rm2 = G.remove('invalid');
assert(rm2 && rm2.ok === false, '不存在 remove 拒');
var skipR = G.build('sect_refund', { skipPay: true });
var rmSkip = G.remove(skipR.gardenId, { skipRefund: true });
assert(rmSkip && rmSkip.ok && rmSkip.refund === 0, 'skipRefund 不退');
assert(T.GARDEN_REFUND_RATE === 0.5, '退费率 0.5');
...
var poor = { currency: { spiritStones: 10 } };
mockWindow.inventory = poor;
var fPay = G.build('sect_poor');
assert(!fPay.ok && fPay.reason === 'spiritStones-low', '灵石不足拒');
...
for (var pi = 0; pi < 100; pi++) {
    var b = G.build('perf_' + pi, { skipPay: true });
    if (b.ok) G.addBeast(b.gardenId, 'beast_perf_' + pi);
}
T.triggerTide('tide_10');
var t0 = Date.now();
