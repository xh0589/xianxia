...
function buyHouse(type) {
    ...
    saveHouseData();
    try {
        if (window.CaveFacilities && typeof window.CaveFacilities.ensureCave === 'function') {
            var map = { cave: 'grass_hut', courtyard: 'stone_room', mansion: 'spirit_manor', palace: 'immortal_manor' };
            window.CaveFacilities.ensureCave('player', map[type] || 'grass_hut');
        }
    } catch (eCave) {}
...
function getHouseStatusHtml() {
    ...
    try {
        if (window.CaveFacilities && typeof window.CaveFacilities.getState === 'function') {
            var cave = window.CaveFacilities.getState().caves && window.CaveFacilities.getState().caves.player;
            if (cave && cave.history && cave.history.length) {
                status += '<p class="text-sm text-cyan-400 font-bold mt-3 mb-1">🪵 近日琐事</p>';
                var hist = cave.history.slice(0, 5);
                for (var hi = 0; hi < hist.length; hi++) {
                    status += '<p class="text-xs text-gray-400">第' + hist[hi].day + '日 · ' + (hist[hi].text || '') + '</p>';
                }
            }
        }
    } catch (eHist) {}
