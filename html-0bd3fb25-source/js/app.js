...
function showBattleUI(battle) {
    ...
    currentBattle.onEnd = (winner) => {
        ...
        if (winner === 'player' && currentBattle) {
            ...
            if (window.currentCharData) {
                // 精力真气清空（兽潮清剿失败已在 settle 里扣过气血，这里仍耗尽余力）
...
function gatherHerbs() {
    ...
    try {
        if (window.BeastEcosystem && typeof window.BeastEcosystem.getActiveBeastBuff === 'function') {
            coldMul = 1 + (window.BeastEcosystem.getActiveBeastBuff('coldHerb') || 0);
        }
    } catch (eCold) {}
    if (coldMul > 1) {
        results.forEach(function (r) {
            if (r.item === 'mat_snow_lotus' || r.item === 'mat_ice_herb') r.chance *= coldMul;
        });
    }
...
function renderBeastList() {
    ...
    for (var i = 0; i < window.tamedBeasts.length; i++) {
        ...
        html += '<div class="bg-gray-700/30 p-4 rounded-lg border border-gray-600">' +
            ...
            '<span class="font-bold text-white">' + b.name + '</span><span class="text-xs text-green-400 ml-2">Lv.' + level + '</span>' + housedNote + '</div>' +
    ...
    if (scoutEl) {
        ...
        if (canScout && window.DungeonDynamic && typeof window.DungeonDynamic.listScouted === 'function') {
            ...
            if (found.length) {
                ...
                scoutEl.innerHTML = '<p class="text-xs text-gray-500">🦅 雷鹰盘旋一圈，暂未探得开启中的秘境窗口。</p>';
            }
        } else {
            scoutEl.innerHTML = '';
        }
