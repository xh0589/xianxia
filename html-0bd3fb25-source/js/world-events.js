...
function expireWorldEvents(gameDay) {
    ...
    expired.forEach(function(id) {
        ...
        if (id === 'beast_tide' && window.BeastTide && typeof window.BeastTide.endTide === 'function') {
            try {
                var st = window.BeastTideAndGarden && window.BeastTideAndGarden.getState && window.BeastTideAndGarden.getState();
                if (st && st.tides) {
                    Object.keys(st.tides).forEach(function (tid) {
                        var t = st.tides[tid];
                        if (t && t.opts && t.opts.source === 'world-event') window.BeastTide.endTide(tid);
                    });
                }
            } catch (eEnd) {}
        }
...
function activateWorldEvent(ev, gameDay) {
    ...
    if (ev.id === 'beast_tide' && window.BeastTide && typeof window.BeastTide.triggerTide === 'function') {
        try {
            var boost = (typeof window.getRealmIndex === 'function' && window.currentCharData)
                ? window.getRealmIndex(window.currentCharData.realm)
                : 0;
            var level = 'tide_1';
            if (boost >= 6) level = 'tide_7';
            else if (boost >= 4) level = 'tide_5';
            else if (boost >= 2) level = 'tide_3';
            window.BeastTide.triggerTide(level, { duration: ev.duration, source: 'world-event' });
        } catch (eTide) {}
    }
    if (ev.id === 'market_boom' && window.MarketDynamic && typeof window.MarketDynamic.applyWorldEvent === 'function') {
        try { window.MarketDynamic.applyWorldEvent('festival'); } catch (eM) {}
    }
    if (ev.id === 'sect_war' && window.MarketDynamic && typeof window.MarketDynamic.applyWorldEvent === 'function') {
        try { window.MarketDynamic.applyWorldEvent('war_start'); } catch (eW) {}
    }
...
function participateWorldEvent(eventId) {
    ...
    if (action === 'fight_beast_tide') {
        if (typeof window.beginBeastTideRaid === 'function') {
            return window.beginBeastTideRaid();
        }
        if (typeof window.startBattle === 'function') {
            ...
            if (window.showMessage) window.showMessage('🐾 你踏入兽群，第一波扑上来了。', 'warning');
        } else if (window.showMessage) {
            window.showMessage('战斗系统未就绪，无法清剿。', 'error');
        }
        return true;
    }
...
function getWorldEventsPanelHtml() {
    ...
    list.forEach(function(ev) {
        ...
        if (ev.id === 'beast_tide' && window.BeastTide && window.BeastTide.isRaidActive && window.BeastTide.isRaidActive()) {
            html += getBeastTideDetailHtml();
        }
        html += '</div>';
    });
    if (window.BeastTide && window.BeastTide.isRaidActive && window.BeastTide.isRaidActive() && !list.some(function (e) { return e.id === 'beast_tide'; })) {
        html += '<div class="bg-gray-700/40 p-3 rounded mb-2 border border-amber-700">' +
            '<span class="font-bold text-amber-300">🐾 山野兽潮（日程）</span>' +
            getBeastTideDetailHtml() + '</div>';
    }
    return html;
}

function getBeastTideDetailHtml() {
    if (!window.BeastTide) return '';
    var t = window.BeastTide.getActiveTide && window.BeastTide.getActiveTide();
    var pool = window.BeastTide.getCurrentPool ? window.BeastTide.getCurrentPool() : [];
    var boost = window.BeastTide.getRarityBoost ? window.BeastTide.getRarityBoost() : 0;
    var rareNames = {
        thunder_eagle: '雷鹰', dragon_turtle: '龙龟', fire_phoenix: '火凤',
        xuan_gui: '玄龟', thunder_beast: '雷兽', crane: '仙鹤',
        black_bear: '黑熊', five_color_deer: '五色鹿', golden_crow: '金乌', kunpeng: '鲲鹏'
    };
    var rares = [];
    for (var i = 0; i < pool.length; i++) {
        if (rareNames[pool[i]]) rares.push(rareNames[pool[i]]);
    }
    var html = '<p class="text-xs text-amber-200 mt-2">' +
        (t ? ('正在进行：' + (t.name || '兽潮') + '。') : '兽潮进行中。') +
        '遭遇更密，收服略易。稀有出没：' + (rares.length ? rares.join('、') : '寻常兽群密度上升') +
        '。稀有度 +' + boost + '</p>';
    var raid = window._tideRaid;
    if (raid && raid.waves) {
        html += '<p class="text-xs text-orange-300 mt-1">清剿进度：第 ' + raid.wave + ' / ' + raid.waves + ' 波' +
            (raid.cores ? ' · 已得兽核 ' + raid.cores : '') + '。败了会带伤中断。</p>';
    } else {
        html += '<p class="text-xs text-gray-500 mt-1">清剿按稀有度分多波，打赢才给本波赏；中途败则带伤中断。</p>';
    }
    if (window.WorldCalendar && typeof window.WorldCalendar.list === 'function') {
        var now = (typeof window.getAbsoluteDay === 'function') ? window.getAbsoluteDay() : 0;
        var due = window.WorldCalendar.list({ category: 'world_event', fromDay: now, toDay: now + 90 }) || [];
        var tideDue = due.filter(function (e) { return e.source && e.source.system === 'beast-tide'; })[0];
        if (tideDue) {
            html += '<p class="text-xs text-gray-400 mt-1">日程：第 ' + tideDue.dueAbsoluteDay + ' 日潮息' +
                (raid && raid.waves ? ' · 清剿第 ' + raid.wave + '/' + raid.waves + ' 波' : '') + '。</p>';
        }
    }
    return html;
}
...
function getCombinedShopPriceMultiplier(cityName, itemId) {
    ...
    if (window.MarketDynamic && typeof window.MarketDynamic.priceMul === 'function') {
        try {
            var mapped = (window.WorldLoop && typeof window.WorldLoop.mapMarketCity === 'function')
                ? window.WorldLoop.mapMarketCity(cityName)
                : cityName;
            var cat = itemId || null;
            m *= (window.MarketDynamic.priceMul(mapped, cat) || 1);
        } catch (eM) {}
    }
    return m;
}
function getCombinedTravelRiskMultiplier(fromCity, toCity) {
    ...
    if (window.BeastTide && typeof window.BeastTide.isRaidActive === 'function' && window.BeastTide.isRaidActive()) {
        var b = window.BeastTide.getRarityBoost ? window.BeastTide.getRarityBoost() : 1;
        m *= 1 + 0.3 * b;
    }
...
if (typeof window !== 'undefined') {
    ...
    window.getBeastTideDetailHtml = getBeastTideDetailHtml;
}
