...
(function (global) {
    ...
    function getRetreatDailyYield() {
        ...
        try {
            if (global.CaveFacilities && typeof global.CaveFacilities.getBuff === 'function') {
                var med = global.CaveFacilities.getBuff('player', 'breakthroughBoost') || 0;
                if (med) bonus *= (1 + med);
            }
        } catch (eMed) {}
    ...
    function runRetreatLoop(plannedDays, opts) {
        ...
        if (global.showMessage) {
            ...
            if (typeof global.showModal === 'function' && summary) {
                var html = '<div class="text-sm text-gray-300 whitespace-pre-line leading-relaxed">' +
                    summary.replace(/</g, '&lt;') + '</div>' +
                    '<p class="text-xs text-gray-500 mt-3">完整日程仍可在「世界日程」里查看这几天的归档。</p>' +
                    '<div class="mt-3 flex flex-wrap gap-2">' +
                    '<button onclick="if(window.openMedicineShop)window.openMedicineShop();" class="text-xs bg-green-700 hover:bg-green-600 text-white px-3 py-1 rounded">去药铺看时价</button>' +
                    '<button onclick="if(window.switchPanel)window.switchPanel(\'calendar\');" class="text-xs bg-gray-700 hover:bg-gray-600 text-white px-3 py-1 rounded">打开世界日程</button>' +
                    '</div>';
                global.showModal('🪷 出关见闻', html);
            }
        }
