...
function tryRegisterTamedBeast(index) {
    ...
    try {
        ...
        if (window.BeastEvolution && typeof window.BeastEvolution.initBeast === 'function') {
            var lineMap = { spirit_fox: 'line_fox', wind_wolf: 'line_fox', fire_phoenix: 'line_phoenix', dragon_turtle: 'line_dragon' };
            window.BeastEvolution.initBeast(bid, lineMap[beast.templateId] || 'line_fox');
        }
...
function canCaptureInCurrentLocation(templateId) {
    ...
    if (window.BeastTide && typeof window.BeastTide.isRaidActive === 'function' && window.BeastTide.isRaidActive()) {
        ...
        var aliases = {
            thunder_eagle: 'thunder_eagle', dragon_turtle: 'dragon_turtle', fire_phoenix: 'fire_phoenix',
            beast_thundereagle: 'thunder_eagle', beast_dragonturtle: 'dragon_turtle', beast_firephoenix: 'fire_phoenix',
            beast_lingfox: 'spirit_fox', beast_windwolf: 'wind_wolf', beast_icesnake: 'ice_serpent'
        };
        if (pool.indexOf(templateId) >= 0 || pool.indexOf(aliases[templateId]) >= 0) return true;
        for (var pi = 0; pi < pool.length; pi++) {
            if (aliases[pool[pi]] === templateId) return true;
        }
    }
    return false;
}
...
function captureBeast(templateId) {
    ...
    if (window.BeastTide && typeof window.BeastTide.getRarityBoost === 'function' && window.BeastTide.isRaidActive && window.BeastTide.isRaidActive()) {
        chance += 0.05 * (window.BeastTide.getRarityBoost() || 0);
    }
...
function trainBeast(index) {
    ...
    var trainGain = 10;
    try {
        var sectId = (window.discipleState && window.discipleState.isInSect && window.discipleState.sectId) || '玩家宗门';
        if (window.BeastGarden && typeof window.BeastGarden.getBuff === 'function') {
            var gb = window.BeastGarden.getBuff(sectId);
            if (gb && gb.trainingPct) trainGain = Math.round(trainGain * (1 + gb.trainingPct));
        }
        if (window.CaveFacilities && typeof window.CaveFacilities.getBuff === 'function') {
            var cb = window.CaveFacilities.getBuff('player', 'beastTraining') || 0;
            if (cb) trainGain = Math.round(trainGain * (1 + cb));
        }
    } catch (eBuff) {}
    beast.exp += trainGain;
    ...
    try {
        if (window.BeastEvolution && typeof window.BeastEvolution.addExp === 'function') {
            var evoId = beast.uid || (beast.templateId + '_' + index);
            window.BeastEvolution.addExp(evoId, trainGain);
            if (typeof window.BeastEvolution.setAffection === 'function') {
                window.BeastEvolution.setAffection(evoId, beast.affection);
            }
        }
    } catch (eEvo) {}
