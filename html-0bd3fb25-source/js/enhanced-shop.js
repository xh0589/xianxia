...
class Shop {
    ...
    buyItem(itemId, quantity = 1) {
        ...
        try {
            if (window.MarketDynamic && typeof window.MarketDynamic.adjustFromTrade === 'function') {
                var _buyCity = (window.locationSystem && window.locationSystem.getCurrentLocation && window.locationSystem.getCurrentLocation()) || '';
                var _mapped = (window.WorldLoop && window.WorldLoop.mapMarketCity) ? window.WorldLoop.mapMarketCity(_buyCity) : _buyCity;
                var _cat = window.MarketDynamic.getItemCategory ? window.MarketDynamic.getItemCategory(realId) : null;
                if (_cat) window.MarketDynamic.adjustFromTrade(_mapped, _cat, quantity, true);
            }
        } catch (eAdj) {}
        try {
...
const TradeService = {
    ...
    executeSell: function(quoteId) {
        ...
        try {
            if (window.MarketDynamic && typeof window.MarketDynamic.adjustFromTrade === 'function') {
                var shopObj = window.shopManager ? window.shopManager.getShop(quote.shopId) : null;
                var loc = shopObj && shopObj.location;
                var mapped = (window.WorldLoop && window.WorldLoop.mapMarketCity) ? window.WorldLoop.mapMarketCity(loc) : loc;
                var cat = window.MarketDynamic.getItemCategory ? window.MarketDynamic.getItemCategory(quote.templateId) : null;
                if (cat) window.MarketDynamic.adjustFromTrade(mapped, cat, quote.quantity, false);
            }
        } catch (eSellAdj) {}
        // 清理报价
        delete this._quotes[quoteId];
                    ...
                    ${(shop.inventory || []).map(item => {
                        ...
                        try {
                            if (window.MarketDynamic && typeof window.MarketDynamic.priceMul === 'function') {
                                var loc = (window.locationSystem && window.locationSystem.getCurrentLocation && window.locationSystem.getCurrentLocation()) || shop.location || '';
                                var mapped = (window.WorldLoop && window.WorldLoop.mapMarketCity) ? window.WorldLoop.mapMarketCity(loc) : loc;
                                var cat = window.MarketDynamic.getItemCategory ? window.MarketDynamic.getItemCategory(item.id) : null;
                                var mul = cat ? window.MarketDynamic.priceMul(mapped, cat) : 1;
                                if (mul <= 0.85) biasNote = '<span class="text-xs text-green-400">此地偏贱 ×' + mul.toFixed(2) + '</span>';
                                else if (mul >= 1.15) biasNote = '<span class="text-xs text-red-400">此地偏贵 ×' + mul.toFixed(2) + '</span>';
                                else if (cat) biasNote = '<span class="text-xs text-gray-500">时价 ×' + mul.toFixed(2) + '</span>';
                            }
                        } catch (eBias) {}
                        return `
                        <div class="flex items-center justify-between bg-gray-800 p-3 rounded border border-gray-600 ${soldOut ? 'opacity-50' : ''}">
                            <div>
                                <div class="font-bold text-gray-200">${item.icon || ''} ${item.name} ${limited} ${biasNote}</div>
