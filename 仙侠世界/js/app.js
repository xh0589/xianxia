...
function renderBeastList() {
    ...
    for (var i = 0; i < window.tamedBeasts.length; i++) {
        ...
        html += '<div class="bg-gray-700/30 p-4 rounded-lg border border-gray-600">' +
            ...
            '<span class="font-bold text-white">' + b.name + '</span><span class="text-xs text-green-400 ml-2">Lv.' + level + '</span></div>' +
            '<p class="text-xs text-gray-400">亲密度: ' + aff + '（' + affTier + '） | 技能: ' + (b.skills?.join(', ') || '无') + tags + tags2 + '</p>' +
