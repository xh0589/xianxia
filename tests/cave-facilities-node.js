...
var fs = require('fs');
var src = fs.readFileSync('' + (process.env.XIANXIA_ROOT || __dirname + '/..') + '/js/extensions/cave-facilities.js', 'utf8');
...
// 无同伴的 cave：v20.0 后会触发空府琐事（4 条 EMPTY_CAVE_EVENTS 概率 0.04-0.08，100 天 ≥ 1）
C.getState().caves['cave_4'] = { level: 'spirit_manor', facilities: [], companions: [], history: [] };
C.install('cave_4', 'fac_spirit_gathering', 0);
listeners = [];
for (var dj = 0; dj < 200; dj++) C.tickDay('cave_4');
var emptyListeners = (listeners['cave:dailyEvent'] || []).filter(function (e) { return e.event && e.event.empty === true; });
assert(emptyListeners.length >= 1, '空府琐事 ≥ 1 (got ' + emptyListeners.length + ')');
