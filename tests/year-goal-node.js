...
mockWindow.showModal = function (title) { mockWindow._showModalCalled = true; };
...
// ============ C2 choose ============
mockWindow.discipleState = freshDs(0);   // v20.x：仅掌门/副掌门可定年度目标
setDay(1);
...
// v20.x：普通弟子(rank 5) 调 choose 应被拒
mockWindow.discipleState = freshDs(5);
var r_perm = mockWindow.SectYearGoal.choose('expand_territory');
assert(r_perm === false, '普通弟子 choose 应被拒');
var st_perm = mockWindow.SectYearGoal._getStore()['少林寺'];
assert(st_perm.goalId === 'cultivate_disciples', '普通弟子不能覆盖目标');
...
// ============ C2b 权限：promptChooseYearGoal 仅掌门/副掌门 ============
// 重置少林寺年度状态，避免"本年度已选"提前返回
var _sg = mockWindow.SectYearGoal._getStore()['少林寺'];
if (_sg) { _sg.year = 0; _sg.goalId = null; }
mockWindow.discipleState = freshDs(0);   // 掌门
mockWindow._showModalCalled = false;
setDay(1);
var r_prom_lead = mockWindow.SectYearGoal.promptChooseYearGoal('少林寺');
assert(r_prom_lead === true, '掌门 prompt 弹选择框');
assert(mockWindow._showModalCalled === true, '掌门 弹出 modal');

// 掌门已选后，再次 prompt 不重复弹（year===curYear && goalId）
mockWindow._showModalCalled = false;
mockWindow.SectYearGoal.choose('expand_territory');
var r_prom_again = mockWindow.SectYearGoal.promptChooseYearGoal('少林寺');
assert(r_prom_again === false, '已选本年不重复弹');
assert(mockWindow._showModalCalled === false, '已选本年不弹 modal');

// 普通弟子：即使未选也不弹
if (_sg) { _sg.year = 0; _sg.goalId = null; }
mockWindow._showModalCalled = false;
mockWindow.discipleState = freshDs(5);   // 普通弟子
var r_prom_disc = mockWindow.SectYearGoal.promptChooseYearGoal('少林寺');
assert(r_prom_disc === false, '普通弟子 prompt 拒绝');
assert(mockWindow._showModalCalled === false, '普通弟子 不弹 modal');

// 副掌门：可弹
if (_sg) { _sg.year = 0; _sg.goalId = null; }
mockWindow._showModalCalled = false;
mockWindow.discipleState = freshDs(1);   // 副掌门
var r_prom_vice = mockWindow.SectYearGoal.promptChooseYearGoal('少林寺');
assert(r_prom_vice === true, '副掌门 prompt 弹选择框');
assert(mockWindow._showModalCalled === true, '副掌门 弹出 modal');
