...
var mockWindow = {
    ...
    WorldLoop: { mapMarketCity: function (loc) { return '中州'; } },
    // helpers
...
// 剩余任务#3：出关时价快照写入 currentCharData._retreatMarket
var _snap = mockWindow.currentCharData && mockWindow.currentCharData._retreatMarket;
assert(!!_snap, '出关应写 _retreatMarket 快照');
assert(_snap && typeof _snap.muls['丹药'] === 'number', '快照含丹药时价');
assert(_snap && _snap.muls['丹药'] === 1.2, '快照丹药时价 = 1.2');
assert(_snap && _snap.day >= 8, '快照记录出关日');
