...
function participateWorldEvent(eventId) {
    ...
    if (action === 'fight_beast_tide') {
        // v20.0：调清剿多波（beginBeastTideRaid 决定分几波）
...
function getWorldEventsPanelHtml() {
    ...
    var html = '';
    if (list.length === 0) {
        // v20.0：列表为空但 BeastTide 还 active（世界事件层结束但 v19.20 兽潮未到期）也要展示
        if (window.BeastTide && window.BeastTide.isRaidActive && window.BeastTide.isRaidActive()) {
            html += '<div class="bg-gray-700/40 p-3 rounded mb-2 border border-amber-700"><span class="font-bold text-amber-300">🐾 兽潮（日程）</span>' +
                getBeastTideDetailHtml() + '</div>';
        } else {
            html += '<p class="text-gray-500 text-sm text-center">当前没有进行中的世界事件</p>';
        }
        return html;
    }
...
// v20.0：兽潮详情 HTML（含稀有出没/清剿波次/潮息日）
function getBeastTideDetailHtml() {
    ...
    if (raid && raid.waves) {
        ...
        html += '<p class="text-xs text-gray-500 mt-1">清剿按稀有度分多波，打赢才给本波赏赐；中途败则带伤中断。</p>';
    }
    ...
    if (window.MarketDynamic && typeof window.MarketDynamic.priceMul === 'function') {
        try {
            var mapped = (window.WorldLoop && typeof window.WorldLoop.mapMarketCity === 'function')
                ...
                : cityName;
            m *= (window.MarketDynamic.priceMul(mapped, itemId || null) || 1);
        } catch (eM) {}
    }
    return m;
}
