...
(function () {
    ...
    // v20.0：空府琐事（没同伴/没事件时随机出一条，让空洞府也"有动静"）
    var EMPTY_CAVE_EVENTS = [
        { id: 'empty_dust',         text: '空府落了一层薄尘，檐下有山风穿过', prob: 0.08 },
        { id: 'empty_spirit_leak',  text: '无人看守，洞府灵气略有散逸',       prob: 0.06 },
        { id: 'empty_wild_herb',    text: '石缝里自己长出一株野药',           prob: 0.04 },
        { id: 'empty_beast_pass',   text: '远处有兽嚎，空府门扉轻轻一震',     prob: 0.05 }
    ];
    ...
    function tickDay(caveId) {
        ...
        // v20.0：没同伴/没事件时随机出空府琐事（空洞府也有动静）
        if (events.length === 0 && cave.companions.length === 0) {
            for (var j = 0; j < EMPTY_CAVE_EVENTS.length; j++) {
                var ee = EMPTY_CAVE_EVENTS[j];
                if (Math.random() < ee.prob) {
                    events.push({ id: ee.id, text: ee.text, empty: true, day: _today() });
                    if (cave.history.length < 20) {
                        cave.history.unshift({ day: _today(), eventId: ee.id, text: ee.text });
                    }
                    if (window.EventBus) window.EventBus.emit('cave:dailyEvent', { caveId: caveId, event: events[events.length - 1] });
                    break;
                }
