...
function updateDaoCompanionDeep() {
    ...
    for (var npcId in bonds) {
        ...
        if (bond.type === 'dao_companion') {
            ...
            if (!npc) continue;
            // 批四：需求初值 30/30/30（推导心情恰为 70，与旧默认一致）——需求增长、心情推导、
            // 孤单提醒统一移交 sect-kin.js 日钩（单一真源，避免双写）；此处只做初始化与 lastInteraction 锚定。
...
function openDaoCompanionPanel() {
    ...
    for (var id in bonds) {
        ...
        var moodWord = (typeof window.daoMoodWord === 'function') ? window.daoMoodWord(id) : '';
        var needHint = (typeof window.daoNeedHints === 'function') ? window.daoNeedHints(id) : '';
        rows.push({ id: id, name: (npc && npc.name) || b.name || id, aff: aff, love: love, level: Number(b.level) || 1, heart: Number(b.bondHeart) || 0, moodWord: moodWord, needHint: needHint });
    }
    var listHtml;
    if (!rows.length) {
        ...
        listHtml = rows.map(function (r) {
            ...
            var moodLine = r.moodWord ? ('<div class="text-xs mt-1"><span class="text-pink-300">心情：' + r.moodWord + '</span>' + (r.needHint ? '<span class="text-gray-500"> · TA' + r.needHint + '</span>' : '') + '</div>') : '';
            return '<div class="bg-gray-900/50 p-3 rounded border border-gray-700 mb-2">'
                ...
                + '<button onclick="window.daoCompanionTalk && window.daoCompanionTalk(\'' + r.id + '\'); window.openDaoCompanionPanel && window.openDaoCompanionPanel();" class="bg-sky-800 hover:bg-sky-700 text-gray-100 text-xs px-3 py-1 rounded">💬 交心（时辰1刻）</button>'
                + '<button onclick="window.daoCompanionGift && window.daoCompanionGift(\'' + r.id + '\'); window.openDaoCompanionPanel && window.openDaoCompanionPanel();" class="bg-amber-800 hover:bg-amber-700 text-gray-100 text-xs px-3 py-1 rounded">🎁 赠礼（灵石50）</button>'
                + '<button onclick="window.daoDateAccept && window.daoDateAccept({npcId:\'' + r.id + '\'}); window.openDaoCompanionPanel && window.openDaoCompanionPanel();" class="bg-rose-900 hover:bg-rose-800 text-gray-200 text-xs px-3 py-1 rounded">🛶 相伴半日</button>'
                + '<button onclick="window.dualCultivate && window.dualCultivate(\'' + r.id + '\'); window.openDaoCompanionPanel && window.openDaoCompanionPanel();" class="bg-pink-700 hover:bg-pink-600 text-white text-xs font-bold px-3 py-1 rounded">💞 双修（精力20' + (r.love >= 50 ? '·深情×1.5' : '') + '）</button>'
                + '</div>'
